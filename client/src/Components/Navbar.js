    ...
    const delayDebounce = setTimeout(async () => {
      try {
        const res = await axios.get(
          `http://localhost:5000/api/employees/search/${encodeURIComponent(
            searchQuery,
          )}`,
          {
            headers: {
    ...
    <div className="container-fluid">
      ...
      >
        <img
          ...
          style={{ width: "35px", height: "35px", objectFit: "contain" }}
        />
        <span className="d-none d-sm-inline me-0 me-md-4" style={{ fontSize: "1.35rem", color: "#404969", fontWeight: "bold" }}>
          Employee Management System
        </span>
      </a>
        ...
        <div className="ms-auto d-flex flex-row flex-sm-row align-items-start align-items-sm-center gap-2 gap-sm-3 w-100">
          ...
          {userData.role === "admin" && (
            <div className="d-flex flex-row  flex-sm-row w-100 w-sm-auto gap-2 gap-sm-3">
              ...
              {/* Search box */}
              <div className="position-relative  mt-2 m-sm-0 flex-grow-1 flex-sm-grow-0" style={{ minWidth: "200px" }}>
                <input
